...
To run the one that doen't require it, simply download it anf run the following command in terminal:
java -jar "%PATH_TO_JAR%", where "%PATH_TO_JAR%" is the place where you store the downloaded file.
This JAR is heavy (~50Mb), as it stores some JavaFX library inside it. Moreover, it doesn't matter what OS you use,
as in that JAR JavaFX for different OS is stored.
